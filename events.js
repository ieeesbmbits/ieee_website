...
export const eventsData = [
  ...
  {
    id: "ieee-sps-scholarship-guidance",
...
export function renderEvents() {
      ...
      <article class="event-row">
        ...
        <div class="event-info-wrap">
          <div class="event-meta">
            <span class="event-date-badge"><i class="fa-regular fa-calendar-days"></i> ${ev.date}</span>
          </div>
  ...
  container.querySelectorAll('.event-img-wrap, .btn-poster').forEach(el => {
    el.addEventListener('click', (e) => {
      ...
      if (!isNaN(idx) && eventsData[idx]) {
        const ev = eventsData[idx];
        if (typeof window.openLightbox === 'function') {
          window.openLightbox(ev.image, ev.title, ev.date, ev.description);
        }
      }
    });
  });
...
// Global fallback for script tag compatibility
